...
function onDrawYAxis () {
  ...
  if (config.yBgColor) {
    ...
    config.yctx.fillStyle= config.yBgColor
  ...
  // 绘制y轴
  config.yctx.beginPath()
  config.yctx.strokeStyle="#DFDEDE"
  config.yctx.lineWidth=0.2
  ...
  // 绘制轴上数值
  config.yctx.beginPath()
  config.yctx.fontSize= 12
  config.yctx.fillStyle= '#CBCDD5'
...
function onDrawXAxis () {
  ...
  if (config.mainBgColor) {
    ...
    config.mainctx.fillStyle= config.mainBgColor
  ...
  // 绘制 x轴
  config.mainctx.beginPath()
  config.mainctx.fillStyle= '#DFDEDE'
  ...
  // 绘制轴上数值
  config.mainctx.beginPath()
  config.mainctx.fontSize= 12
  config.mainctx.fillStyle= '#CBCDD5'
  ...
  config.mainctx.strokeStyle = '#DFDEDE'
  config.mainctx.lineWidth= 1
...
function onForDrawKLines (i, yNumpx, open, close, high, low, startX, startY, endX, endY, lowpx, highpx) {
  ...
  if (open < close) {
    // 上涨
    config.mainctx.fillStyle= config.upcolor
    config.mainctx.strokeStyle = config.upcolor
    ...
    // 下跌
    config.mainctx.fillStyle= config.downcolor
    config.mainctx.strokeStyle = config.downcolor
    ...
    // 平
    config.mainctx.fillStyle= '#666'
    config.mainctx.strokeStyle = '#666'
    startY = config.maincHeight - config.xHeight - (open - config.minNum) * yNumpx - config.yMargin / 2
    endY = config.maincHeight - config.xHeight - (open - config.minNum) * yNumpx - config.yMargin / 2
  }
...
function onDrawKLineBorder (isScroll = false) {
  ...
  config.optctx.strokeStyle = '#333'
  config.optctx.lineWidth= 2
...
function onDrawTradeSign (kIndex, i) {
  ...
  config.mainctx.beginPath()
  config.mainctx.fontSize= 12
  config.mainctx.fillStyle= '#fff'
  config.mainctx.fillText(signType, (centerX - r / 2), centerY + r / 3)
  config.mainctx.stroke()

  config.mainctx.beginPath()
  config.mainctx.fontSize= 13
  config.mainctx.fillStyle= '#333'
